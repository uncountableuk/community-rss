...
- `article-processor.ts` extracts summary by passing HTML through `sanitize-html` with `allowedTags: []` to produce plain text, then normalises whitespace and caps at 200 chars.
...
- Infinite scroll uses offset-based pagination (page/limit) rather than cursor-based, matching the articles API design. **Tech debt:** flagged for migration to cursor-based (`WHERE published_at < ?`) before 0.4.0 to prevent duplicate items when new articles shift offsets during active syncing.
- `env.d.ts` extended with optional `CF_ACCESS_CLIENT_ID` and `CF_ACCESS_CLIENT_SECRET` fields for Zero Trust support.

### 0.2.0 Release Review Feedback (Post-Release)

The following feedback was received from the release review and has been
assessed and incorporated into the appropriate documents:

1. **Pagination Tech Debt (High Priority):** Offset-based pagination is an
   infinite-scroll anti-pattern with actively syncing feeds — new articles
   shift offsets, causing duplicates. Cursor-based pagination
   (`WHERE published_at < ?`) recommended before 0.4.0. Added to
   PROJECT_PLAN.md Risk Register.

2. **Cloudflare Pages vs Workers:** Pages *does* support Cron Triggers and
   Queues in production via "Advanced Mode" `_worker.js` output. The inline
   sync workaround (`POST /api/v1/admin/sync`) risks breaching the 30s HTTP
   timeout with large feeds in production. Recommendation: deploy to a real
   CF Pages Preview environment before 0.6.0 to verify handlers. Added to
   PROJECT_PLAN.md Risk Register.

3. **Summary extraction method:** The implementation notes incorrectly
   described summary extraction as "regex stripping" — the actual code uses
   `sanitize-html` with `allowedTags: []`, which is safe and handles HTML
   entities. Implementation notes corrected. The `cheerio` dependency
   (already installed) remains reserved for image extraction in 0.6.0.

4. **System User & Future Profiles:** The `ensureSystemUser()` fix was
   praised. For 0.5.0, the UI and API must distinguish "Global/System Feeds"
   (system user), "Admin Feeds" (admin user), and "Personal Feeds" (verified
   author). Added to PROJECT_PLAN.md Risk Register and 0.3.0 plan.

5. **FreshRSS ClientLogin (Praised):** The two-step auth flow and token
   caching were correctly implemented. Code should remain heavily commented.
   Verified: auth token is cached for the `FreshRssClient` instance lifetime.
