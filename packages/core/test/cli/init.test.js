...
describe('CLI scaffold', () => {
    ...
    describe('scaffold()', () => {
        ...
        it('should create page files with correct imports', () => {
            ...
            const indexPage = readFileSync(join(tempDir, 'src/pages/index.astro'), 'utf-8');
            expect(indexPage).toContain('@community-rss/core');
        });
        it('should create email templates with variable placeholders', () => {
            scaffold({ cwd: tempDir });
            const signIn = readFileSync(join(tempDir, 'src/email-templates/sign-in.html'), 'utf-8');
            expect(signIn).toContain('{{');
            expect(signIn).toContain('<!-- subject:');
        });
        it('should create astro.config.mjs with node adapter', () => {
            scaffold({ cwd: tempDir });
            const config = readFileSync(join(tempDir, 'astro.config.mjs'), 'utf-8');
        ...
        it('should skip existing files without --force', () => {
            ...
            writeFileSync(join(tempDir, 'src/pages/index.astro'), 'custom content');
            const { created, skipped } = scaffold({ cwd: tempDir });
            expect(skipped).toContain('src/pages/index.astro');
            expect(created).not.toContain('src/pages/index.astro');
            // Verify original content preserved
            const content = readFileSync(join(tempDir, 'src/pages/index.astro'), 'utf-8');
        ...
        it('should throw when no package.json found', () => {
            ...
            expect(() => scaffold({ cwd: emptyDir })).toThrow('Could not find package.json');
            rmSync(emptyDir, { recursive: true, force: true });
        });
        it('should create nested directory structures', () => {
            scaffold({ cwd: tempDir });
            expect(existsSync(join(tempDir, 'src/pages/auth/signin.astro'))).toBe(true);
            expect(existsSync(join(tempDir, 'src/pages/article/[id].astro'))).toBe(true);
            expect(existsSync(join(tempDir, 'src/email-templates/welcome.html'))).toBe(true);
        });
        ...
        it('should create theme.css with CSS custom property overrides', () => {
            scaffold({ cwd: tempDir });
            const theme = readFileSync(join(tempDir, 'src/styles/theme.css'), 'utf-8');
