...
import { startScheduler, stopScheduler, isSchedulerRunning, } from '@utils/build/scheduler';
describe('Scheduler', () => {
    const mockApp = {
        db: {},
        config: {
    ...
    describe('startScheduler', () => {
        it('should register a cron job with the configured schedule', () => {
            ...
            expect(mocks.mockSchedule).toHaveBeenCalledWith('*/30 * * * *', expect.any(Function));
        });
        ...
        it('should call syncFeeds when cron fires', async () => {
            ...
            let cronCallback;
            mocks.mockSchedule.mockImplementation((_schedule, cb) => {
                cronCallback = cb;
