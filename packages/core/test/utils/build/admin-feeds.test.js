...
describe('Admin Feeds', () => {
    describe('validateFeedUrl', () => {
        ...
        it('should reject URLs that return non-200 status', async () => {
            mocks.mockFetch.mockResolvedValue(new Response('Not Found', { status: 404 }));
        ...
        it('should reject URLs that return non-RSS content', async () => {
            mocks.mockFetch.mockResolvedValue(new Response('<html><body>Not a feed</body></html>', {
                status: 200,
        ...
        it('should accept valid RSS feed by content-type', async () => {
            mocks.mockFetch.mockResolvedValue(new Response('<rss><channel><title>My Blog</title></channel></rss>', {
                status: 200,
        ...
        it('should accept valid Atom feed by body markers', async () => {
            mocks.mockFetch.mockResolvedValue(new Response('<feed xmlns="http://www.w3.org/2005/Atom"><title>Atom Blog</title></feed>', {
                status: 200,
    ...
    describe('submitAdminFeed', () => {
        ...
        it('should create an approved feed when URL is valid', async () => {
            mocks.mockFetch.mockResolvedValue(new Response('<rss><channel><title>Admin Feed</title></channel></rss>', {
                status: 200,
                headers: { 'Content-Type': 'application/rss+xml' },
            }));
            ...
            expect(mocks.mockUpsertFeed).toHaveBeenCalledWith(mockDb, expect.objectContaining({
                userId: 'user-admin-1',
                feedUrl: 'https://example.com/feed.xml',
                title: 'Admin Feed',
                status: 'approved',
        ...
        it('should use custom title when provided', async () => {
            mocks.mockFetch.mockResolvedValue(new Response('<rss><channel><title>Original</title></channel></rss>', {
                status: 200,
                headers: { 'Content-Type': 'application/rss+xml' },
            }));
            mocks.mockUpsertFeed.mockResolvedValue([{
            ...
            expect(mocks.mockUpsertFeed).toHaveBeenCalledWith(mockDb, expect.objectContaining({ title: 'My Custom Title' }));
        });
        it('should throw when feed URL is invalid', async () => {
            await expect(submitAdminFeed(mockDb, 'user-admin-1', 'not-a-url')).rejects.toThrow('Invalid URL format');
        });
        it('should throw when feed URL returns non-RSS content', async () => {
            mocks.mockFetch.mockResolvedValue(new Response('<html></html>', {
                status: 200,
                headers: { 'Content-Type': 'text/html' },
            }));
            await expect(submitAdminFeed(mockDb, 'user-admin-1', 'https://example.com/page')).rejects.toThrow('RSS or Atom');
        });
        it('should generate deterministic IDs for deduplication', async () => {
            mocks.mockFetch.mockResolvedValue(new Response('<rss><channel><title>Blog</title></channel></rss>', {
                status: 200,
                headers: { 'Content-Type': 'application/rss+xml' },
            }));
...
//# sourceMappingURL=admin-feeds.test.js.map