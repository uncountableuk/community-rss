...
import {
    ...
    createSmtpTransport,
} from '@utils/build/email-transports';
import type { EmailMessage } from '@core-types/email';
    ...
    describe('createResendTransport', () => {
        ...
        it('should throw on non-2xx Resend response', async () => {
            ...
            await expect(transport.send(testMessage)).rejects.toThrow(
                'Resend email failed (422): Validation error',
            );
        });
    ...
    describe('createSmtpTransport', () => {
        ...
        it('should log warning on non-2xx response but not throw', async () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith(
                expect.stringContaining('Mailpit send failed (500)'),
            );
            warnSpy.mockRestore();
        });
