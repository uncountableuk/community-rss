...
describe('POST /api/v1/auth/signup', () => {
    ...
    it('should send sign-in link and return exists:true when account already exists', async () => {
        ...
        const mockHandler = vi.fn().mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200 }));
        ...
        const handlerCallArg = mockHandler.mock.calls[0][0];
        const body = await handlerCallArg.json();
    ...
    it('should create pending signup and send magic link on success', async () => {
        ...
        const data = (await response.json());
        expect(data.success).toBe(true);
        expect(mocks.mockCreatePendingSignup).toHaveBeenCalledWith(expect.anything(), 'new@example.com', 'New User');
        expect(mockHandler).toHaveBeenCalled();
    });
    it('should return 500 when magic link send fails', async () => {
        ...
        const mockHandler = vi.fn().mockResolvedValue(new Response('error', { status: 500 }));
        mocks.mockCreateAuth.mockReturnValue({ handler: mockHandler });
        const request = createPostRequest({
            email: 'new@example.com',
            name: 'New User',
