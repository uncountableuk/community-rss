...
describe('POST /api/v1/profile/change-email', () => {
    ...
    it('should return 401 when not authenticated', async () => {
        mocks.mockRequireAuth.mockRejectedValue(new Response(JSON.stringify({ code: 'UNAUTHORIZED' }), {
            status: 401,
            headers: { 'Content-Type': 'application/json' },
        }));
        const request = new Request('http://localhost/api/v1/profile/change-email', {
    ...
    it('should set pending email and send verification on valid request', async () => {
        ...
        expect(mocks.mockSetPendingEmail).toHaveBeenCalledWith(mockApp.db, mockUsers.registered.id, 'new@example.com', 'test-uuid-1234', expect.any(Date));
        expect(mocks.mockSendEmailChangeEmail).toHaveBeenCalledWith(mockApp, 'new@example.com', expect.stringContaining('/auth/verify-email-change?token=test-uuid-1234'), undefined, expect.objectContaining({
...
//# sourceMappingURL=change-email.test.js.map