...
describe('auth catch-all route', () => {
    ...
    const createContext = (url, options = {}) => ({
    ...
    it('should handle magic link verification with guest migration', async () => {
        ...
        const response = await ALL(ctx);
        expect(mockMigrateGuestToUser).toHaveBeenCalledWith(mockApp.db, 'guest-uuid-123', 'new-user-1');
...
//# sourceMappingURL=catch-all.test.js.map