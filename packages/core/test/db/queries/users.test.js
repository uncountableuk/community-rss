...
const { mockAll, mockOnConflictDoNothing, mockRun, mockValues, mockInsert, mockUpdate, mockDelete, mockSet, mockSelect, } = vi.hoisted(() => {
...
import { SYSTEM_USER_ID, ensureSystemUser, getUserById, getUserByEmail, createGuestUser, migrateGuestToUser, updateUser, getUserByPendingEmailToken, setPendingEmail, confirmEmailChange, isAdmin, isSystemUser, } from '@db/queries/users';
...
describe('users queries', () => {
    ...
    describe('updateUser', () => {
        ...
        it('should update termsAcceptedAt', async () => {
            ...
            expect(mockUpdate).toHaveBeenCalled();
            expect(mockSet).toHaveBeenCalledWith(expect.objectContaining({ termsAcceptedAt: now }));
            expect(result).toEqual(updated);
        });
    ...
    describe('setPendingEmail', () => {
        it('should set pending email fields on the user', async () => {
            ...
            mockAll.mockResolvedValueOnce([updated]);
            const result = await setPendingEmail(mockDb, mockUsers.registered.id, 'new@example.com', 'tok-xyz', expiresAt);
            expect(mockUpdate).toHaveBeenCalled();
            expect(mockSet).toHaveBeenCalledWith(expect.objectContaining({
                pendingEmail: 'new@example.com',
                pendingEmailToken: 'tok-xyz',
        ...
        it('should return null if user not found', async () => {
            mockAll.mockResolvedValueOnce([]);
            const result = await setPendingEmail(mockDb, 'nonexistent', 'new@example.com', 'tok-xyz', new Date());
            expect(result).toBeNull();
        });
    });
