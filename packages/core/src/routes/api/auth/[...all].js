 ...
 * @since 0.3.0
 */
export const ALL = async ({ request, locals }) => {
    ...
    try {
        ...
        try {
            ...
            return new Response(JSON.stringify({ error: 'Authentication failed', details: errorMsg }), { status: 500, headers: { 'Content-Type': 'application/json' } });
        }
            ...
            try {
                ...
                if (session?.user) {
                    ...
                    if (guestId) {
                        ...
                        newResponse.headers.set('Set-Cookie', 'crss_guest=; Path=/; Max-Age=0; SameSite=Lax');
                        return newResponse;
                    }
                }
            }
        ...
        return new Response(JSON.stringify({ error: 'Unexpected authentication error' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
    }
};
 ...
 */
function getGuestIdFromCookie(request) {
