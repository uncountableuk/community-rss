...
export const POST = async ({ request, locals }) => {
    ...
    try {
        ...
        if (!newEmail) {
            return new Response(JSON.stringify({ code: 'INVALID_EMAIL', message: 'Email address is required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        // Basic email format validation
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(newEmail)) {
            return new Response(JSON.stringify({ code: 'INVALID_EMAIL', message: 'Invalid email address format' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        // Cannot be the same as the current email
        const currentEmail = session.user.email?.toLowerCase();
        if (newEmail === currentEmail) {
            return new Response(JSON.stringify({ code: 'SAME_EMAIL', message: 'New email address must differ from the current one' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        // Check whether the new address is already taken
        const existing = await getUserByEmail(app.db, newEmail);
        if (existing) {
            return new Response(JSON.stringify({ code: 'EMAIL_IN_USE', message: 'That email address is already in use' }), { status: 409, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        return new Response(JSON.stringify({ message: `Verification link sent to ${newEmail}` }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
    catch (err) {
