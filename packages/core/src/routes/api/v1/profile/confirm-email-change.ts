...
export const GET: APIRoute = async ({ request, locals }) => {
    ...
    if (!token) {
        return new Response(
            JSON.stringify({ code: 'MISSING_TOKEN', message: 'Missing verification token' }),
    ...
    try {
        ...
        if (result === null) {
            return new Response(
                JSON.stringify({ code: 'TOKEN_NOT_FOUND', message: 'Invalid or already-used verification token' }),
        ...
        if ('expired' in result && result.expired) {
            return new Response(
                JSON.stringify({ code: 'TOKEN_EXPIRED', message: 'This verification link has expired. Please request a new one from your profile.' }),
                { status: 400, headers: { 'Content-Type': 'application/json' } },
            );
        }

        return new Response(
            JSON.stringify({ message: 'Email address updated successfully' }),
