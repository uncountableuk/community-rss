...
export const GET = async ({ request, locals }) => {
    ...
    try {
        ...
        if (!user) {
            return new Response(JSON.stringify({ code: 'USER_NOT_FOUND', message: 'User not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
        }
    ...
    try {
        ...
        if (body.name !== undefined) {
            ...
            if (!name || name.length < 1) {
                return new Response(JSON.stringify({ code: 'INVALID_NAME', message: 'Display name cannot be empty' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
            }
            if (name.length > 100) {
        ...
        if (body.bio !== undefined && body.bio !== null) {
            if (body.bio.length > 500) {
                return new Response(JSON.stringify({ code: 'INVALID_BIO', message: 'Bio must be 500 characters or less' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
            }
        }
        const updateData = {};
        if (body.name !== undefined)
        ...
        if (Object.keys(updateData).length === 0) {
            return new Response(JSON.stringify({ code: 'NO_CHANGES', message: 'No fields to update' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
