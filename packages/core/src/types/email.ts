 ...
 * @since 0.3.0
 */
export type EmailTemplateFunction<TData = Record<string, unknown>> =
    (context: EmailTemplateContext, data: TData) => EmailContent;

// ─── Built-in Email Type Data Payloads ───────────────────────
...
export interface EmailService {
    ...
    send<K extends string>(
        type: K,
        to: string,
        data: K extends keyof EmailTypeDataMap ? EmailTypeDataMap[K] : Record<string, unknown>,
        profile?: EmailUserProfile,
    ): Promise<void>;
}
