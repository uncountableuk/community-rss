 ...
 * @since 0.2.0
 */
export function setArticleList(ids) {
    articleIds = ids;
}
/**
 ...
 * @since 0.2.0
 */
export async function openArticleModal(articleId) {
    currentIndex = articleIds.indexOf(articleId);
    // Update URL without page reload
    const previousUrl = window.location.pathname;
    window.history.pushState({ articleId, previousUrl }, '', `/article/${articleId}`);
    try {
 ...
 * @since 0.2.0
 */
export function navigateArticle(direction) {
...
function renderModal(article) {
  ...
  `;
    ...
    container.querySelector('.crss-modal-overlay')?.addEventListener('click', (e) => {
        if (e.target.classList.contains('crss-modal-overlay')) {
            closeArticleModal();
        }
    });
