 ...
 * @since 0.3.0
 */
export function resolveTransport(env, emailConfig) {
    ...
    if (transportName === 'resend') {
        ...
        if (!apiKey) {
            console.warn('[community-rss] Email transport is "resend" but RESEND_API_KEY is not set. Emails will not be sent.');
            return null;
        }
    ...
    console.warn(`[community-rss] Unknown email transport "${transportName}". Emails will not be sent.`);
    return null;
}
/**
 ...
 * @since 0.3.0
 */
export function resolveTemplate(type, emailConfig) {
 ...
 * ```
 */
export function createEmailService(app) {
    ...
    return {
        async send(type, to, data, profile) {
            if (!transport) {
                console.warn(`[community-rss] No email transport configured. Skipping "${type}" email to ${to}.`);
                return;
            }
            // Resolution order:
            ...
            const customTemplate = emailConfig?.templates?.[type];
            if (customTemplate) {
                const context = { appName, email: to, profile };
                const content = customTemplate(context, data);
            ...
            if (!template) {
                console.warn(`[community-rss] No template registered for email type "${type}". Skipping email to ${to}.`);
                return;
            }
            const context = { appName, email: to, profile };
