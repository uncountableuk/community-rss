 ...
 * @since 0.3.0
 */
export async function requireAuth(request, app) {
    const auth = createAuth(app);
    const session = await auth.api.getSession({
        headers: request.headers,
    });
    if (!session) {
        throw new Response(JSON.stringify({ code: 'AUTH_REQUIRED', message: 'Authentication required' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
    }
    return session;
}
/**
 ...
 * @since 0.3.0
 */
export async function requireAdmin(request, app) {
    const session = await requireAuth(request, app);
    const userRole = session.user.role;
    if (userRole !== 'admin') {
        throw new Response(JSON.stringify({ code: 'ADMIN_REQUIRED', message: 'Admin access required' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
    }
    return session;
}
